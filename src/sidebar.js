...
var ResearchAgentSidebar = {
  ...
  mount(body, doc, initialItem) {
    ...
    style.textContent = `
      ...
      .research-agent-session-drawer { display:none; flex:1; flex-direction:column; gap:8px; min-height:0; padding:10px; border:1px solid var(--ra-border); border-radius:10px; background:color-mix(in srgb,var(--ra-accent) 4%,transparent); }.research-agent-session-drawer.is-open{display:flex}.research-agent-session-list{display:flex;flex:1;flex-direction:column;gap:5px;min-height:300px;overflow:auto}.research-agent-session-item{display:flex;flex-direction:column;align-items:flex-start;gap:2px;width:100%;text-align:left}.research-agent-session-item.is-active{border-color:var(--ra-accent);background:var(--ra-accent-weak)}.research-agent-session-item small{color:var(--fill-secondary,#687583)}.research-agent-session-actions{display:flex;flex-wrap:wrap;gap:6px}.research-agent-session-toggle{display:flex;align-items:center;gap:6px;color:var(--fill-secondary,#687583);font-size:.88em}
