...
1. Use the included `research-agent-0.2.0.xpi` (or create it with the packaging command below).
...
node tests/indexer-contract.test.js && node tests/semantic-contract.test.js && node tests/jobs-contract.test.js && node tests/agent-stream-contract.test.js && node tests/prefs-models-contract.test.js
zip -X -r research-agent-0.2.0.xpi manifest.json bootstrap.js prefs.js prefs.xhtml prefs-ui.js chrome src locale icons LICENSE README.md
unzip -t research-agent-0.2.0.xpi
```
