...
1. Use the included `research-agent-0.3.7.xpi` (or create it with the packaging command below).
...
4. Go to Zotero **Settings → Research Agent**, enter each service's API URL and key, fetch its model list from the provider, select models, and run the connection test. Brave Search is optional; GitHub Code Search requires an access token.
...
zip -X -r research-agent-0.3.7.xpi manifest.json bootstrap.js prefs.js prefs.xhtml prefs-ui.js chrome src locale icons vendor LICENSE README.md
unzip -t research-agent-0.3.7.xpi
```
